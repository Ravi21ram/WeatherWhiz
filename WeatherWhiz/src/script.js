function main() {
  ...
  function showFoodAndPlaces(city) {
    // Replace with your preferred Food API endpoint
    let foodApi = `https://example.com/food-api/v1/search?q=${city}`;
let placesApi = `https://example.com/places-api/search?city=${city}`;


    // Make API calls for food and places (handle errors appropriately)
    fetch(foodApi)
        .then(response => response.json())
        .then(foodData => {
            // Process and display food data (e.g., popular dishes, restaurants)
            let foodList = document.getElementById("foodList");
            foodList.innerHTML = ""; // Clear previous content
            foodData.forEach(foodItem => {
                let listItem = document.createElement("li");
                listItem.textContent = foodItem.name; // Assuming 'name' is the property that holds the food name
    ...
    fetch(placesApi)
        .then(response => response.json())
        .then(placesData => {
            // Process and display tourist place data (e.g., attractions, landmarks)
            let placesList = document.getElementById("placesList");
            placesList.innerHTML = ""; // Clear previous content
            placesData.forEach(place => {
                let listItem = document.createElement("li");
                listItem.textContent = place.name; // Assuming 'name' is the property that holds the place name
                placesList.appendChild(listItem);
            });
        })
        .catch(error => console.error("Error fetching tourist place data:", error));
}
